...
public class InventoryHighlight : MonoBehaviour
{
    ...
    public void SetSize(InventoryItem targetItem)
    {
        ...
        size.x = targetItem.Width * Inventory.tileSizeWidth;
        size.y = targetItem.Height * Inventory.tileSizeHeight;

        highlighter.sizeDelta = size;
    }
    
    /** 하이라이트 이미지 위치 설정한다 (인벤토리에 이미 존재하는 아이템) */
    public void SetPosition(Inventory targetGrid, InventoryItem targetItem)
    {
    ...
    public void SetPosition(Inventory targetGrid, InventoryItem targetItem, int posX, int posY)
    {
        // 아이템이 위치할 그리드의 좌표를 받아서 중심을 계산한다
    ...
    public void SetParent(Inventory targetGrid)
    {
        // 인벤토리가 없을 경우 종료
